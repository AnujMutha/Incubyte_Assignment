...
public class StringCalculator {
    ...
    int add(String numbers){
        ...
        for(int i = 0; i < tokens.length; ++i) {
            ...
            //Checking whether the token is a negative number or not,
            //if yes then adding it to the list of negative numbers amd setting the flag as true hence an exception can be raised`
            if (tokens[i].matches(".*-\\d.*")) {
                negNumbers.add(Integer.parseInt(tokens[i]));
                negFlag = true;
            }

            //Checking whether the token is a number or not if yes then adding it to the resultant variable
            ...
            //if yes then identifying the standard occurring positions of the alphabet and then adding position in the resultant sum
                ...
                //To get the standard position of the alphabet in the english alphabet table,
                //we subtract 96 from the ascii value of current char in order to get the position of the alphabet
                /* The following intuition works because
                *  the lowercase a - z starts from 97 and if we subtract 96 from we get position
                * For Example : a = 97 - 96 = 1
                *               b = 98 - 96 = 2
                * */
                int alphabetPosition = c - 96;

                //Finally, adding the position into resultant variable
